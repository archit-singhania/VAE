...
class _MediaScreenState extends State<MediaScreen> {
  ...
  Future<void> generate() async {
    ...
    if (token == null || workspace == null || prompt.text.trim().length < 3) {
      return;
    }
    setState(() => generating = true);
    try {
      final created = await widget.state.client
          .generateImage(token, workspace.id, prompt.text.trim());
      widget.state.prependAssets(created);
      prompt.clear();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Media added to your library.')));
      }
    } catch (error) {
      widget.state.reportError(error);
    } finally {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (context, _) => ListView(
          ...
          children: [
            ...
            const Text(
                'Create an image now, then publish it to connected channels.',
                style: TextStyle(color: AevraColors.textSoft)),
                  ...
                  children: [
                    ...
                    TextField(
                        ...
                        decoration: const InputDecoration(
                            hintText: 'A premium product launch image for…')),
            ...
            if (widget.state.assets.isEmpty)
              const Text('Your generated and uploaded media will appear here.',
                  style: TextStyle(color: AevraColors.muted))
            else
              ...widget.state.assets.map((asset) => ListTile(
